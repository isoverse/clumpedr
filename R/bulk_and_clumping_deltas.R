...
bulk_and_clumping_deltas  <- function(.data, ...,
  ...
  out <- .data %>%
    ...
    mutate(R13_wg  = .env$R13_PDB * (1 + .data$d13C_PDB_wg / 1000),
           R18_wg  = .env$R18_PDBCO2 * (1 + .data$d18O_PDBCO2_wg / 1000)) %>%
    ...
    mutate(R45 = (1 + {{ d45 }} / 1000) * .data$R45_wg,
           ...
           # Taylor polynomial:
           K = exp(.env$D17O / 1000) * .env$R17_PDBCO2 * .env$R18_PDBCO2 ^ (-lambda),

           A = -3 * .data$K^2 * .env$R18_PDBCO2^(2 * lambda),
           B = 2 * .data$K * .data$R45 * .env$R18_PDBCO2^lambda,
           C = 2 * .env$R18_PDBCO2,
           D = -.data$R46,
           ...
           d18O_PDB = (.data$d18O_PDBCO2 + 1000) / .env$R18_PDB - 1000,

           R18 = (1 + .data$d18O_PDBCO2 / 1000) * .env$R18_PDBCO2,
