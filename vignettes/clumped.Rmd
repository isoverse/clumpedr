...
I've made some standard data available here so as to run the tests. You can
play around with those here:

```{r}
?standards
```

or a single did file for an ETH-3 standard
...
```{r}
didinfo  <- clean_did_info(standards, masspec = "MOTU")
```
...
```{r}
library(plotly)
toWebGL(ggplotly(dynamicTicks = TRUE))
```
...
```{r}
dati <- coll %>%
  add_info(iso_get_file_info(didinfo))
```
