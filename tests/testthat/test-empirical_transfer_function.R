test_that("append_expected_values() works", {
  ...
  expected_test <- standards %>%
    ...
    add_info(isoreader::iso_get_file_info(clean_did_info(standards, "MOTU")), cols = "Identifier 1") %>%
...
##     add_info(iso_get_file_info(clean_did_info(standards, "MOTU")), cols = c("file_id", "Analysis")) %>%
##     find_outliers() %>%
##     append_expected_values() %>%
##     calculate_etf()
...
test_that("empirical transfer function wrapper works", {
  ...
  ## tst <- standards %>%
  ##   clean_did_info("MOTU") %>%
  ##   iso_get_raw_data(include_file_info = c("file_id", "Analysis")) %>%
  ##   mutate(dis_min = 500, dis_max = 50000, dis_fac = 3) |>
  ##   find_bad_cycles(min = "dis_min", max = "dis_max", fac = "dis_fac", relative_to = "init") |>
  ##   spread_match() %>%
  ##   append_ref_deltas(standards) %>%
  ##   delta_values() |>
  ##   ## find_outliers()
  ##   collapse_cycles(cols = c(d18O_PDBCO2, d13C_PDB, D47_raw),
  ##                   id = c("file_id", "Analysis")) |>
  ##   ## empirical_transfer_function()
  ##   # TODO

  ## expect_s3_class(tst, "tbl_df")
})
